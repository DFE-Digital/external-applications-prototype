...
const data = require('../data/data-alpha-sprint-21');

module.exports = function (router) {

    var version = "alpha-sprint-21";
    ...
    router.post('/' + version + '/incoming-trust-confirmation-handler', function (req, res) {
        ...
        if (confirmTrust === 'yes') {
            ...
            req.session.data['progressive-enhancement-selected-trust'] = req.session.data.tempSelectedTrust;
        }
    ...
    router.post('/' + version + '/application-complete', function (req, res) {
        ...
        if (application) {
            ...
            application.trustDetails = req.session.data['new-trust'] === 'yes' 
                ...
                : { type: 'Existing trust', details: req.session.data['progressive-enhancement-selected-trust'] };
    ...
    router.post('/' + version + '/declaration-form-handler', function (req, res) {
        ...
        const declarationData = {
            ...
            equalitiesImpact: req.body['equalitiesImpact'] || '',
            status: 'Signed'
        };
    ...
    router.get('/' + version + '/declaration-summary', function (req, res) {
        ...
        // Create list of outgoing trusts from Details of Trusts task
        let uniqueOutgoingTrusts = [];
        if (req.session.data['outgoing-trusts'] && req.session.data['outgoing-trusts'].length > 0) {
            uniqueOutgoingTrusts = req.session.data['outgoing-trusts'].map(trust => trust.name);
        }
        
        // Process incoming trust status
        let incomingTrustStatus = 'Not signed yet';
        if (req.session.data['declarations'] && req.session.data['progressive-enhancement-selected-trust']) {
            const incomingDeclaration = req.session.data['declarations'].find(declaration => 
                declaration.trust.name === req.session.data['progressive-enhancement-selected-trust'].name
            );
