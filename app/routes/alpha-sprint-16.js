...
module.exports = function (router) {
    ...
    // Handle member search results page
    router.get('/' + version + '/members-search-results', function (req, res) {
        const radioItems = data.members.map(member => ({
        ...
        res.render(version + '/members-search-results', {
            radioItems: radioItems
        });
    });

    // Handle member selection
    router.post('/' + version + '/members-confirmation', function (req, res) {
        // Store the selected member in session
        req.session.data['selected-member'] = req.body['selected-member'];
        
        res.render(version + '/members-confirmation', {
            data: req.session.data
        });
    });

    // Handle member confirmation
    router.post('/' + version + '/add-more-members-handler', function (req, res) {
        const confirmMember = req.body['confirm-member'];
        
        if (confirmMember === 'yes') {
            // Get the selected member details
            const selectedMember = req.session.data['selected-member'];
            const [name, role] = selectedMember.split('|||');
            ...
            // Clear any existing errors since we now have a member
            delete req.session.data.errors;
        }
        
        // Go to member current responsibilities
        return res.redirect('member-current-responsibilities');
    });

    // GET handler for member current responsibilities
    router.get('/' + version + '/member-current-responsibilities', function (req, res) {
    ...
    router.post('/' + version + '/member-current-responsibilities', function (req, res) {
        ...
        req.session.data['member-current-responsibilities'] = req.body['member-current-responsibilities'];
        
        res.redirect('member-past-responsibilities');
    });

    // GET handler for member past responsibilities
    router.get('/' + version + '/member-past-responsibilities', function (req, res) {
    ...
    router.post('/' + version + '/member-past-responsibilities', function (req, res) {
        // Store the past responsibilities
        req.session.data['member-past-responsibilities'] = req.body['member-past-responsibilities'];
        
        res.redirect('member-future-role');
    });

    // GET handler for member future role
    router.get('/' + version + '/member-future-role', function (req, res) {
    ...
    router.post('/' + version + '/member-future-role', function (req, res) {
        // Store the future role
        req.session.data['member-future-role'] = req.body['member-future-role'];
        
        res.redirect('add-member-to-list');
    });

    // Handle adding member to list
    router.post('/' + version + '/add-member-to-list', function (req, res) {
        // Get the selected member details
        const selectedMember = req.session.data['selected-member'];
        const [name, role] = selectedMember.split('|||');

        // Find the member in the list and update with additional details
        if (req.session.data['members-to-add']) {
            ...
            if (memberIndex !== -1) {
                ...
                req.session.data['members-to-add'][memberIndex].futureRole = req.session.data['member-future-role'];
            }
        }
        ...
        delete req.session.data['selected-member'];
        
        res.redirect('members-summary');
    });

    // Handle member to remove search results page
    router.get('/' + version + '/members-to-remove-search-results', function (req, res) {
        const radioItems = data.members.map(member => ({
            value: member.name + "|||" + member.role,
            text: member.name,
            hint: {
        ...
        res.render(version + '/members-to-remove-search-results', {
            radioItems: radioItems
        });
    });

    // Handle member to remove selection
    router.post('/' + version + '/members-to-remove-confirmation', function (req, res) {
        // Store the selected member in session
        req.session.data['selected-member-to-remove'] = req.body['selected-member-to-remove'];
        
        res.render(version + '/members-to-remove-confirmation', {
            data: req.session.data
        });
    });

    // Handle member to remove confirmation
    router.post('/' + version + '/add-member-to-remove-list', function (req, res) {
        const confirmMember = req.body['confirm-member-to-remove'];
        
        if (confirmMember === 'yes') {
            // Get the selected member details
            const selectedMember = req.session.data['selected-member-to-remove'];
            const [name, role] = selectedMember.split('|||');
            ...
            // Add the new member to the removal list
        ...
        // Clear temporary data
        delete req.session.data['selected-member-to-remove'];
        
        res.redirect('members-summary');
    });

    // Handle the confirm delete member page parameters
    router.get('/' + version + '/confirm-delete-member', function (req, res) {
        req.session.data['delete-member-index'] = req.query.index;
        res.render(version + '/confirm-delete-member');
    });

    // Handle member deletion
    router.post('/' + version + '/delete-member', function (req, res) {
        const confirmDelete = req.body['confirm-delete-member'];
        const memberIndex = req.session.data['delete-member-index'];

        if (confirmDelete === 'yes' && req.session.data['members-to-add']) {
    ...
    // Handle the confirm delete member to remove page parameters
    router.get('/' + version + '/confirm-delete-member-to-remove', function (req, res) {
        req.session.data['delete-member-to-remove-index'] = req.query.index;
        res.render(version + '/confirm-delete-member-to-remove');
    });

    // Handle member to remove deletion
    router.post('/' + version + '/delete-member-to-remove', function (req, res) {
        const confirmDelete = req.body['confirm-delete-member-to-remove'];
        const memberIndex = req.session.data['delete-member-to-remove-index'];

        if (confirmDelete === 'yes' && req.session.data['members-to-remove']) {
            // Remove the member at the specified index
